...
def cal_dice(hist):
    if hist[1][1] == 0:
        return 0
    dice = 2 * hist[1][1] / (hist[0][1] + hist[1][0] + 2 * hist[1][1])
    return dice



def seg(norm, mask, k, filename, result_save_path):
    norm = norm / 255.
    norm = cv2.blur(norm, (3, 3))
    norm[norm < k] = int(0)
    norm[norm >= k] = int(1)
    ...
    print(filename, f)
    # 保存测试数据到csv文件
    item = {'img': filename, 'F1-score': str(f), 'IoU': str(IoU), 'MIoU': str(MIoU), 'Precision': str(p), 'Recall': str(r), 'Dice': str(dice)}
    fieldnames = ['img', 'F1-score', 'IoU', 'MIoU', 'Precision', 'Recall', 'Dice']
    save_result = os.path.join(result_save_path, str(k) + '.csv')
...
def seg_for_ods(mask_path, res_path, result_save_path):
    for i in range(100):
        threshold = 0.01 + i * 0.01
        print("Threshold: {:.2f}".format(threshold))
        for res in os.listdir(res_path):
            # if int(res.split('.')[0]) < 71 and int(res.split('.')[0]) > 96:
            #     continue
            ...
            seg(res_img, mask, threshold, res, result_save_path)