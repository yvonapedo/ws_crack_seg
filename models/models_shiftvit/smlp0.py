...
class sMLPNet(nn.Module):
    ...
    def __init__(self, in_chans=3, dim=80, alpha=3, num_classes=1000, patch_size=4, image_size=224, depths=[2,8,14,2], dp_rate=0.,
        ...
        self.conv_layers_80 = nn.Sequential(
            ...
            nn.BatchNorm2d(320),
            nn.ReLU(inplace=True),
            nn.Conv2d(320, 640, kernel_size=8, stride=8),  # Adjust kernel size and stride as needed
            nn.BatchNorm2d(640),
            nn.ReLU(inplace=True),
        )
        self.conv_layers_160 = nn.Sequential(
            nn.Conv2d(160, 320, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(320),
            nn.ReLU(inplace=True),
            nn.Conv2d(320, 640, kernel_size=4, stride=2, padding=1),
            nn.BatchNorm2d(640),
            nn.ReLU(inplace=True)
        )
        self.conv_layers_320 = nn.Sequential(
            nn.Conv2d(320, 640, kernel_size=4, stride=2, padding=1),
            nn.BatchNorm2d(640),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x):
        ...
        # 0 # torch.Size([1, 80, 56, 56]) 250,880
        # 1 # torch.Size([1, 160, 28, 28])125,440
        # 2 # torch.Size([1, 320, 14, 14])62,720
        # 3 # torch.Size([1, 640, 7, 7])31,360

        final = []
        output_80 = self.conv_layers_80(output[0])
        final.append(output_80)
        output_160 = self.conv_layers_160(output[1])
        final.append(output_160)

        output_320 = self.conv_layers_320(output[2])
        final.append(output_320)
        final.append(output[3])
