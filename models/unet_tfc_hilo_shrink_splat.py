...
class Unet_tfc_Hilo_Splat(nn.Module):
    def __init__(self, n_channels, num_classes, bilinear=True):
        super(Unet_tfc_Hilo_Splat, self).__init__()
        ...
        self.TFC_S1 = TFC(in_channel=in_dim[0], out_channel=in_dim[0], img_size=[16, 8], num_patch=128, p_size=1,
                          emb_dropout=0.1,
                          T_depth=tfc_depth[0],
                          heads=heads, dim_head=dim_head, mlp_dim=mlp_dim, dropout=0.1)
        self.TFC_S2 = TFC(in_channel=in_dim[2], out_channel=in_dim[2], img_size=[16, 8], num_patch=128, p_size=1,
                          emb_dropout=0.1,
                          T_depth=tfc_depth[1],
                          heads=heads, dim_head=dim_head, mlp_dim=mlp_dim, dropout=0.1)
        self.TFC_S3 = TFC(in_channel=in_dim[3], out_channel=in_dim[3], img_size=[16, 8], num_patch=128, p_size=1,
                          emb_dropout=0.1,
                          T_depth=tfc_depth[2],
                          heads=heads, dim_head=dim_head, mlp_dim=mlp_dim, dropout=0.1)
